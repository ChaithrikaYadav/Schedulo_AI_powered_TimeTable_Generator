"""
config.py — Three-tier configuration abstraction for ChronoAI.
Automatically selects the correct settings class based on the ENVIRONMENT env variable.
Usage: from chronoai.config import get_settings; settings = get_settings()
"""
...
class LocalSettings(BaseSettings):
    ...
    app_name: str = "ChronoAI"
    app_version: str = "1.0.0"

    # ── Database ───────────────────────────────────────────────────────────────
    db_engine: str = "sqlite"  # sqlite | postgresql
    database_url: str = "sqlite+aiosqlite:///./chronoai.db"
